...
- **Usage:** `usage.input.cachedTokenCount` shows prefix caching within a session. The follow-up step after a tool round reuses almost the whole prompt (183 of 184 input tokens cached). A probe that appended long unrelated turns reported 0 cached tokens.
